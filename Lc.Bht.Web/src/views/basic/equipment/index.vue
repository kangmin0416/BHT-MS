<template>
  <div class="app-container">
    ...
    <el-row>
      <el-table ref="equipment" v-loading="loading" :data="dataEquipment" row-key="id" stripe border :height="tableHeight*0.65" :default-sort="{prop: queryParams.orderby, order: queryParams.sort}" @sort-change="handleSortable">
        <el-table-column type="index" :index="handleIndexCalc" label="#" align="center" />
        <el-table-column sortable prop="equipNo" align="center" :show-overflow-tooltip="true" label="设备编号" width="120" />
        <el-table-column sortable prop="equipName" :show-overflow-tooltip="true" align="center" label="设备名称" />
        <el-table-column sortable prop="lineName" :show-overflow-tooltip="true" align="center" label="所属产线" />
        ...
        <el-table-column label="操作" align="center" width="160" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button-group>
              <el-button v-permission="['PRIV_EQUIPMENT_UPDATE']" type="info" size="mini" icon="el-icon-edit" @click="handleUpdate(scope.row)" />
              <el-button v-permission="['PRIV_EQUIPMENT_DELETE']" type="danger" size="mini" icon="el-icon-delete" @click="handleDelete(scope.row)" />
            </el-button-group>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body :close-on-click-modal="false" @close="cancel">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="设备编号" maxlength="50" prop="equipNo">
          ...
          <el-col :span="24">
            <el-form-item label="所属工序" prop="processUID">
              <el-select v-model="form.processUID" clearable filterable placeholder="请选择" style="width:100%" @change="handleSelectLine">
          ...
          <el-col :span="24">
            <el-form-item label="所属产线" prop="lineUID">
              <el-select v-model="form.lineUID" clearable filterable placeholder="请选择" style="width:100%">
                <el-option v-for="item in filterLine(lineOption,form.processUID)" :key="item.id" clearable :label="item.lineNo +' | '+item.lineName" :value="item.id" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
import { getAllProductline } from '@/api/basic/productline'
export default {
  name: 'equipment',
  data() {
    return {
  ...
  created() {
    ...
    getAllProductline(true).then(response => {
      this.lineOption = response.data
    })
  },
  methods: {
    /** 查询设备列表 */
    getList() {
      this.loading = true
      queryEquipment(this.queryParams).then(response => {
        this.dataEquipment = response.data.dataSource
    ...
    // 过滤产线
    filterLine(data, search) {
      return data.filter(m => m.processUID === search)
    }
  }
}
</script>

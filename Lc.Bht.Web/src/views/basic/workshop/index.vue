<template>
  <div class="app-container">
    <el-row v-if="searchToggle" :gutter="20">
      <el-col>
        <el-form :inline="true" @submit.native.prevent>
          <el-form-item>
            <el-input v-model="queryParams.queryText" placeholder="请输入车间名称" clearable prefix-icon="el-icon-search" @keyup.enter.native="handleQuery" @clear="handleQuery" />
          </el-form-item>
          <el-form-item>
    ...
    <el-row type="flex" class="mb8">
      <el-button v-permission="['PRIV_WORKSHOP_CREATE']" type="success" icon="el-icon-plus" size="mini" @click="handleCreate">新增车间</el-button>
    ...
    <el-row>
      <el-table ref="workshop" v-loading="loading" :data="dataWorkshop" row-key="id" stripe border :height="tableHeight*0.65" :default-sort="{prop: queryParams.orderby, order: queryParams.sort}" @sort-change="handleSortable">
        <el-table-column type="index" :index="handleIndexCalc" label="#" align="center" />
        <el-table-column sortable prop="workShopNo" :show-overflow-tooltip="true" align="center" label="车间编号" width="120" />
        <el-table-column sortable prop="workShopName" :show-overflow-tooltip="true" align="center" label="车间名称" />
        ...
        <el-table-column label="操作" align="center" width="160" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button-group>
              <el-button v-permission="['PRIV_WORKSHOP_UPDATE']" type="info" size="mini" icon="el-icon-edit" @click="handleUpdate(scope.row)" />
              <el-button v-permission="['PRIV_WORKSHOP_DELETE']" type="danger" size="mini" icon="el-icon-delete" @click="handleDelete(scope.row)" />
            </el-button-group>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body :close-on-click-modal="false" @close="cancel">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="车间编号" maxlength="50" prop="workShopNo">
              <el-input v-model="form.workShopNo" placeholder="请输入车间编号" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
          ...
          <el-col :span="24">
            <el-form-item label="所属工厂" prop="factoryUID">
              <el-select v-model="form.factoryUID" clearable filterable placeholder="请选择" style="width:100%">
                <el-option v-for="item in factoryOption" :key="item.id" clearable :label="item.factoryNo +' | '+item.factoryName" :value="item.id" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="车间备注" prop="remark">
              <el-input v-model="form.remark" type="textarea" :autosize="{minRows:2}" maxlength="100" show-word-limit placeholder="请输入车间备注" />
            </el-form-item>
          </el-col>
          <el-col :span="6">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      queryWorkshop(this.queryParams).then(response => {
        this.dataWorkshop = response.data.dataSource
    ...
    handleDelete(row) {
      this.$confirm(
        '是否确认删除名称为"' + row.workShopName + '"的车间?',
        '警告',
        {
        ...
        .then(() => {
          deleteWorkshop(row.id).then(response => {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id !== undefined) {
            ...
            createWorkshop(this.form).then(response => {
              if (response.code === 200) {
                this.$message({
                  message: '新增成功',
                  type: 'success'
